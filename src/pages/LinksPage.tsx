...
export function LinksPage() {
  const { data = [], isLoading } = useQuery({ queryKey: ['public-links'], queryFn: async () => {
    const { data } = await db.from('lilkank_items').select('*, lilkank_categories(name,slug)').eq('item_type', 'link').eq('status', 'published').in('visibility', ['public', 'unlisted', 'members']).order('published_at', { ascending: false })
    return (data || []) as Item[]
  } })
